...
def main(tracker: tracking.BallTracker, controller: positionController_2.Controller, mqtt_client: MQTTClientJetson):
    ...
    try:
        while True:
            ...
            if abs(ori[1]-np.deg2rad(0.7)) < 0.001 and abs(ori[0]) < 0.001 and not reached:
                controller.arduinoThread.send_target_positions(0, 0)
                reached = True
            
            if not reached:
                print(f"{abs(ori[1]-np.deg2rad(0.7))}, {abs(ori[0])}")
                controller.axisControl((np.deg2rad(0.7), 0))
                time.sleep(0.015)
