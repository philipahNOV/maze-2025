...
class Controller:
    ...
    def posControl(self, ref):
        ...
        if self.prevPos is not None:
            ...
            ball_vel = (np.array(self.pos) - np.array(self.prevPos)) / dt
        else:
        ...
        if abs(e_x) < self.pos_tol and abs(edot_x) < self.vel_tol:
            ...
            theta_x = np.sign(e_x) * np.log1p(self.kp_x * abs(e_x))
        else:
            # Ball is far → USE CONTROL
            theta_x = (self.kp_x * e_x  + self.kd_x * ball_vel[0] + self.ki_x * self.e_x_int)
        ...
        if abs(e_y) < self.pos_tol and abs(edot_y) < self.vel_tol:
            ...
            # Ball is close, but needs help moving → ESCAPE DEAD ZONE
            theta_x = -np.sign(e_y) * np.log1p(self.kp_y * abs(e_y))
        else:
            # Ball is far → USE CONTROL
            theta_y = (self.kp_y * e_y  + self.kd_y * ball_vel[0] + self.ki_y * self.e_y_int)
