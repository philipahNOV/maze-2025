...
def global_lab_search(frame, lab_lower, lab_upper):
    """Search for objects using LAB color space instead of HSV"""
    lab = cv2.cvtColor(frame, cv2.COLOR_BGR2LAB)
    mask = cv2.inRange(lab, lab_lower, lab_upper)
    return get_center_of_mass(mask)

def lab_tracking(frame, prev_pos, lab_lower, lab_upper, window_size=80):
    """Track using LAB color space instead of HSV"""
    if prev_pos is None:
        return None
...
# Keep the old HSV functions for backwards compatibility
