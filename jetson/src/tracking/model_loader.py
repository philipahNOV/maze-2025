...
class YOLOModel:
    ...
    def __init__(self, model_path="v8-512.engine", input_shape=(512, 512)):
        ...
        self._preproc_buffer = np.empty((1, 3, *input_shape), dtype=np.float32)
        self._img_fp16 = np.empty((1, 3, *input_shape), dtype=np.float16)
    ...
    def _init_tensorrt(self, model_path):
        self.names = ['ball']
        self.original_h, self.original_w = self.input_shape
        ...
        input_vol = 1 * 3 * self.input_shape[0] * self.input_shape[1]
        output_vol = trt.volume(self.output_shape)

        self.input_host = cuda.pagelocked_empty(input_vol, dtype=self.input_dtype)
        self.output_host = cuda.pagelocked_empty(output_vol, dtype=self.output_dtype)
        ...
        print("[YOLOModel] Initialized TensorRT engine with:", self.input_shape, self.input_dtype)
    ...
    def preprocess(self, image):
        ...
        np.divide(img.astype(np.float32), 255.0, out=img)
        img = np.transpose(img, (2, 0, 1))
        self._preproc_buffer[0] = img
        np.copyto(self._img_fp16[0], self._preproc_buffer[0].astype(np.float16))
        np.copyto(self.input_host, self._img_fp16.ravel())
    ...
    def postprocess(self, output, conf_thres=0.2):
        ...
        assert output.shape[0] == 5, f"Unexpected output shape: {output.shape}"
        ...
        boxes = [self.Box(x1[i], y1[i], x2[i], y2[i], conf[i], cls[i]) for i in range(len(conf))]
