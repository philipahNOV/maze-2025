...
class ballRollingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Penalize for moving away from the goal
        if self.velocity > 0.05 and self.distance_to_goal < 0.05:
        ...
        # Initialize a variable to keep track of whether all distances are under the threshold
        all_distances_under_threshold = True
        
        # Iterate through every third item in self.future_actions to check the distances
        for i in range(2, len(self.future_actions), 3):  # Start at index 2 (the first distance), step by 3
            if abs(self.future_actions[i]) >= 0.01:
                all_distances_under_threshold = False
                self.reward = -2
                break
        
        # If all distances are under 0.01, mark the episode as done
        if all_distances_under_threshold:
            self.reward = 0
            self.steps_at_goal += 1
            if self.steps_at_goal > 100:
                self.done = True
                self.steps_at_goal = 0
        
        return observation, self.reward, self.done, {'goal_position': self.goal_position, 'ball_position': self.ball_position}
...
model = PPO.load("AI/models/model.zip", env=env) # type: ignore
...
for iteration in range(iterations):
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            if step % LOG_STEP == 0:
                wandb.log({"reward": reward, "distance_to_goal": obs[2], "velocity": obs[1], "step": step, "episode": episode, "angle": obs[0], "goal_position": info['goal_position'], "ball_position": info['ball_position']})
            score += reward



model_path = "AI/models/model4.zip"
