...
class ballRollingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if -0.01 < self.distance_to_goal < 0.01 and -0.01 < self.future_actions[-1] < 0.01:
            self.reward = 0
            self.steps_at_goal += 1
            if self.steps_at_goal > 500:
                self.goal_position = random.choice([random.uniform(-0.5, -0.1), random.uniform(0.1, 0.5)])
                self.steps_at_goal = 0
            
        
        return observation, self.reward, self.done, {'goal_position': self.goal_position, 'position': self.ball_position}
...
model = PPO.load("AI/models/model4.zip", env=env) # type: ignore
...
for iteration in range(iterations):
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            if step % LOG_STEP == 0:
                wandb.log({"reward": reward, "distance_to_goal": obs[2], "velocity": obs[1], "step": step, "episode": episode, "angle": obs[0], "goal_position": info['goal_position'], "position": info['position']})
            score += reward
